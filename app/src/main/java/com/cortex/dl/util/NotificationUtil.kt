...
import com.cortex.dl.NotificationActionReceiver
import com.cortex.dl.NotificationActionReceiver.Companion.ACTION_CANCEL_TASK
import com.cortex.dl.NotificationActionReceiver.Companion.ACTION_ERROR_REPORT
import com.cortex.dl.NotificationActionReceiver.Companion.ACTION_KEY
import com.cortex.dl.NotificationActionReceiver.Companion.ERROR_REPORT_KEY
import com.cortex.dl.NotificationActionReceiver.Companion.NOTIFICATION_ID_KEY
import com.cortex.dl.NotificationActionReceiver.Companion.TASK_ID_KEY
...
object NotificationUtil {
    ...
    private const val NOTIFICATION_GROUP_ID = "cortex.download.notification"
    private const val DEFAULT_NOTIFICATION_ID = 100
    const val SERVICE_NOTIFICATION_ID = 123
    private lateinit var serviceNotification: Notification
    // A download notification is intentionally posted only twice: once when it starts
    // and once when it finishes (or fails). Continuous progress notifications make
    // System UI sluggish on some devices and are not needed for this app's UX.
    private val startedDownloadNotifications = ConcurrentHashMap.newKeySet<Int>()

    //    private var builder =
    //        NotificationCompat.Builder(context, CHANNEL_ID).setSmallIcon(android.R.drawable.stat_sys_download)
    ...
    fun notifyProgress(
        ...
        val builder = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .setContentText(displayText ?: "Download started")
            .setOngoing(true)
            .setOnlyAlertOnce(true)
            .setStyle(NotificationCompat.BigTextStyle().bigText(displayText))
        
        pendingIntent?.let {
            builder.addAction(0, context.getString(R.string.cancel), it)
        }
    ...
    fun finishNotification(
        ...
        startedDownloadNotifications.remove(notificationId)
    ...
    fun notifyError(
        ...
        val displayText = errorMessage ?: context.getString(textId)
