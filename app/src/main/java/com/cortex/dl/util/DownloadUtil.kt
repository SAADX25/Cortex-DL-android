...
import java.io.File
import com.cortex.dl.BuildConfig
import com.cortex.dl.App.Companion.audioDownloadDir
import com.cortex.dl.App.Companion.context
import com.cortex.dl.App.Companion.videoDownloadDir
...
// Cookie is defined in com.cortex.dl.util.Cookie (same package — no import needed)
import com.cortex.dl.util.FileUtil.getArchiveFile
import com.cortex.dl.util.FileUtil.getConfigFile
import com.cortex.dl.util.FileUtil.getCookiesFile
import com.cortex.dl.util.FileUtil.getExternalTempDir
import com.cortex.dl.util.FileUtil.getFileName
import com.cortex.dl.util.FileUtil.getSdcardTempDir
import com.cortex.dl.util.makeToast
import com.cortex.dl.util.FileUtil.moveFilesToSdcard
import com.cortex.dl.util.PreferenceUtil.COOKIE_HEADER
...
object DownloadUtil {
    ...
    fun parseCookieContent(profileUrl: String, content: String): List<Cookie> =
        CookieParser.parseCookieContent(profileUrl, content)
    ...
    private fun YoutubeDLRequest.addOptionsForVideoDownloads(
        ...
        this.apply {
            downloadPreferences.run {
                ...
                // Force yt-dlp to clean up separated audio/video files after muxing
    ...
    fun writeDocsTextFile(videoInfo: VideoInfo): Result<String> = runCatching {
        ...
        val content = buildString {
            ...
            appendLine("═══════════════════════════════════════")
            appendLine("  Generated by Cortex DL")
            appendLine("═══════════════════════════════════════")
        }
